...
export default function TranslationView({ onSelectTranslation }: TranslationViewProps) {
  const { contextApp, selectedTranslation, setSelectedTranslation, translationSearchTerm, setTranslationSearchTerm, getFilteredTranslations } = useAppContext();
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 overflow-hidden backdrop-blur-sm max-w-none">
        <table className="w-full text-left text-sm table-fixed">
          ...
          <thead className="bg-slate-900/50 text-slate-400 border-b border-slate-700">
            <tr>
              ...
              <th className="p-4 font-medium">FR</th>
          ...
          <tbody className="divide-y divide-slate-700/50">
            {translations.length === 0 ? (
              <tr>
                <td colSpan={4} className="p-8 text-center text-slate-400">
                  {translationSearchTerm
                    ? `No translations found matching "${translationSearchTerm}"`
              ...
              translations.map((key) => {
                ...
                const enValue = contextApp.translationValues?.en?.[key] || '-';
                const frValue = contextApp.translationValues?.fr?.[key] || '-';
                ...
                return (
                  ...
                  >
                    ...
                    <td className="p-4 text-center">
                      <span className="inline-flex items-center justify-center min-w-6 h-6 px-2 rounded-full bg-slate-800 text-slate-400 text-xs font-bold border border-slate-700">
                        {componentIds.length}
                      </span>
                    </td>
                  </tr>
